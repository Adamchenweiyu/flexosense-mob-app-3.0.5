import 'package:flutter/material.dart';

class AppNavObserver extends NavigatorObserver {
  /// A stack of routes that have been pushed.
  /// NOTE: Only save route with name != null
  /// route with name = null will be ignored (dialog, bottom sheet, etc.)
  static final navStack = <RouteStackItem>[];

  static String? getCurrentRouteName() {
    if (navStack.isEmpty) {
      return null;
    }
    return navStack.last.name;
  }

  @override
  void didPop(Route route, Route? previousRoute) {
    if (previousRoute != null && route.settings.name != null) {
      navStack.removeLast();
    }
    super.didPop(route, previousRoute);
  }

  @override
  void didPush(Route route, Route? previousRoute) {
    if (route.settings.name != null) {
      navStack.add(RouteStackItem.fromRoute(route));
    }
    super.didPush(route, previousRoute);
  }

  @override
  void didRemove(Route route, Route? previousRoute) {
    if (previousRoute != null && route.settings.name != null) {
      navStack.removeLast();
    }
    super.didRemove(route, previousRoute);
  }

  @override
  void didReplace({Route? newRoute, Route? oldRoute}) {
    if (oldRoute != null) {
      navStack.removeLast();
    }
    if (newRoute != null) {
      navStack.add(RouteStackItem.fromRoute(newRoute));
    }
    super.didReplace(newRoute: newRoute, oldRoute: oldRoute);
  }

  @override
  void didStartUserGesture(Route route, Route? previousRoute) {
    super.didStartUserGesture(route, previousRoute);
  }

  @override
  void didStopUserGesture() {
    super.didStopUserGesture();
  }

  static bool isRouteInStack(String routeName) {
    return navStack.any((item) => item.name == routeName);
  }
}

class RouteStackItem {
  final String? name;
  final Object? args;

  const RouteStackItem({
    required this.name,
    required this.args,
  });

  factory RouteStackItem.fromRoute(Route route) =>
      RouteStackItem(name: route.settings.name, args: route.settings.arguments);

  @override
  String toString() => 'RouteStackItem(name: $name, args: $args)';
}
