...
class StoredMagneticBottomSheet extends StatefulWidget {
  final String? title;
  final List<dynamic> data;

  const StoredMagneticBottomSheet({
    Key? key,
    this.title,
    required this.data,
  }) : super(key: key);
  ...
  @override
  State<StoredMagneticBottomSheet> createState() => _StoredDataBottomSheetState();
}

class _StoredDataBottomSheetState extends State<StoredMagneticBottomSheet> {
  final headers = ['Time', 'X', 'Y', 'Z'];
  var _isDeleting = false;
  var _isExportingAll = false;

  Future<void> _exportDataToExcel(List<dynamic> data) async {
  ...
  void _exportAllData() async {
    ...
    dynamic data;
    data = await serviceLocator.get<MagneticRepo>().get().get();
  ...
  void _deleteAllData() async {
    ...
    await serviceLocator.get<MagneticRepo>().deleteAll();
