...
import com.example.feet_sensor_sdk.ble.BleScanner
import com.example.feet_sensor_sdk.ble.BleStateController
import com.example.feet_sensor_sdk.core.handler.DeviceHandler
import com.example.feet_sensor_sdk.core.handler.DeviceHandlerCallBack
...
class CoreHandler private constructor(private val context: Context) : DeviceHandlerCallBack,
    ...
    // Device properties
    private var scannedDeviceList: MutableList<BluetoothDevice> = mutableListOf()
    ...
    fun requestToConnect(address: String) {
        ...
        val scannedDevice = scannedDeviceList.firstOrNull { it.address == address }
        ...
        deviceHandler = DeviceHandler(context, scannedDevice, this)
        deviceHandler.requestToConnect()
    ...
    private fun isDeviceExistedInScanList(device: BluetoothDevice): Boolean {
        if (scannedDeviceList.isEmpty()) return false
        val filterDevice = scannedDeviceList.firstOrNull { it.address == device.address }
