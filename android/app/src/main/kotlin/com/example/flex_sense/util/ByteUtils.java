...
public class ByteUtils {
    ...
    public static byte[] shortsToByteArray(short[] arrays) {
        ByteBuffer byteBuffer = ByteBuffer.allocate(arrays.length * 2);
        ShortBuffer shortBuffer = byteBuffer.asShortBuffer();
        shortBuffer.put(arrays);
    ...
    public static byte[] subByteStartStopArray(byte[] src, int startPos, int endPos) {
        if (startPos < 0|| endPos <= startPos || endPos > src.length) {
            return new byte[0];
        }
        return Arrays.copyOfRange(src, startPos, endPos);
    }
    ...
    /**
     * Compare 2 byte array contain same payloadData
     *
     * @param b1
     * @param b2
     * @return
