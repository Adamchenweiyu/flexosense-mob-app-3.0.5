...
import com.example.flex_sense.plugin.device_core.enum.DeviceCoreMethod
...
//                DeviceCoreMethodHelper.getInstance(mContext).reconnect(args, result)
            ...
            DeviceCoreMethod.SET_START_STOP_STUDY.value -> {
                DeviceCoreMethodHelper.getInstance(mContext).setStartStopStudy(args, result)
            }
            ...
            DeviceCoreMethod.GET_ERROR_CODE.value -> {
                DeviceCoreMethodHelper.getInstance(mContext).getErrorCode(args, result)
            }

            DeviceCoreMethod.GET_BATTERY_INFO.value -> {
                DeviceCoreMethodHelper.getInstance(mContext).getBatteryInfo(args, result)
            }
